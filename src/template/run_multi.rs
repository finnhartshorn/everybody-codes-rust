...
pub mod child_commands {
        ...
        output
            ...
            .for_each(|(part, timing_str, nanos)| {
                if part.contains("Part 1") {
                    ...
                    timings.part_3 = Some(timing_str.into());
                }
    ...
    mod tests {
        ...
        fn parses_execution_times() {
            let res = parse_exec_time(
                &[
                    ...
                    "Part 3: 20 (100µs @ 50000 samples)".into(),
                    "".into(),
                ],
                day!(1),
            );
            assert_approx_eq!(res.total_nanos, 74230074.13_f64);
            assert_eq!(res.part_1.unwrap(), "74.13ns");
            assert_eq!(res.part_2.unwrap(), "74.13ms");
            assert_eq!(res.part_3.unwrap(), "100µs");
        }
        ...
        fn parses_with_patterns_in_input() {
            let res = parse_exec_time(
                &[
                    ...
                    "Part 3: 5s (50ms @ 2 samples)".into(),
                    "".into(),
                ],
                day!(1),
            );
            assert_approx_eq!(res.total_nanos, 2150000000_f64);
            assert_eq!(res.part_1.unwrap(), "2s");
            assert_eq!(res.part_2.unwrap(), "100ms");
            assert_eq!(res.part_3.unwrap(), "50ms");
        }
        ...
        fn parses_missing_parts() {
            ...
            assert_eq!(res.part_3.is_none(), true);
        }
    }
}
